...
func runConfigDoctor(cmd *cobra.Command, args []string) error {
	...
	if address, err := blockchain.ResolveKeyAddress(effective, effective.KeyName); err != nil {
...
func resolveAddressWithPassword(keyName, keyringBackend, passphrase string, keyringDir ...string) string {
	...
	address, err := blockchain.ResolveKeyAddressWithPassphrase(keyringBackend, keyDir, keyName, passphrase)
	if err != nil {
		return ""
	}
