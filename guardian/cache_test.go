...
func TestCacheStateTransitions(t *testing.T) {
	...
	chain.UpdateAssignmentStatus(awaitingSecretID, guardianAddr, "ASSIGNMENT_STATUS_ACCEPTED")
	chain.SetSecretState(awaitingSecretID, "pending")
	mocks.AdvanceToRevealWindow(chain, awaitingSecretID)

	// Update cache
	err = cache.UpdateFromBlockchain(ctx, mockClient, guardianAddr, chain.GetCurrentHeight())
	require.NoError(t, err)

	// Should now be in reveal state
	revealSecrets := cache.GetSecretsNeedingReveal(chain.GetCurrentHeight())
...
func TestCacheIndexedAccess(t *testing.T) {
	...
	mocks.SetupMultiGuardianSecret(chain, 1, guardianAddrs, "awaiting_acceptance")
	mocks.SetupMultiGuardianSecret(chain, 1, guardianAddrs, "awaiting_acceptance")
	mocks.SetupMultiGuardianSecret(chain, 1, guardianAddrs, "pending")
...
func TestCacheInitialization(t *testing.T) {
	...
	mocks.SetupMultiGuardianSecret(chain, 1, guardianAddrs, "awaiting_acceptance")
	mocks.SetupMultiGuardianSecret(chain, 1, guardianAddrs, "pending")
	mocks.SetupMultiGuardianSecret(chain, 1, guardianAddrs, "revealed")
	mocks.SetupMultiGuardianSecret(chain, 1, guardianAddrs, "cancelled")
...
func TestCacheAutomaticEviction(t *testing.T) {
	...
	chain.AdvanceBlocks(10)

	// Update cache to trigger cleanup
	err = cache.UpdateFromBlockchain(ctx, mockClient, mocks.TestGuardianAddress, chain.GetCurrentHeight())
	require.NoError(t, err)
...
func TestCacheRevealWindowValidation(t *testing.T) {
	...
	chain.AddAssignment("test-reveal", mocks.TestGuardianAddress, "ASSIGNMENT_STATUS_ACCEPTED")
...
func TestDetermineLocalState_ReconstructableStillNeedsReveal(t *testing.T) {
	...
	revealed.RevealedShares = []blockchain.RevealedShare{
	...
	pendingRevealed.RevealedShares = []blockchain.RevealedShare{
		{GuardianAddress: mocks.TestGuardianAddress, RevealedAtBlock: 150},
	}
