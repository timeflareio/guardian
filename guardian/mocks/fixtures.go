...
func CreateTestGuardian(address string, available bool) *blockchain.Guardian {
	return &blockchain.Guardian{
		Address:             address,
		EncryptionPublicKey: []byte("0123456789abcdef0123456789abcdef"), // 32 bytes
		AvailableFrom:       1,
		AvailableUntil:      1_000_000,
		Stake: blockchain.Coin{
			Denom:  "uveil",
...
func CreateTestGuardianWithRealKeypair(address string, available bool) (*blockchain.Guardian, [32]byte, error) {
...
func CreateTestSecret(id string, threshold int64, state string) *blockchain.Secret {
	return &blockchain.Secret{
		ID:                  id,
		Creator:             "tmflr1creator123456789",
		State:               state,
		Threshold:           threshold,
		GuardianAssignments: make([]blockchain.GuardianAssignment, 0),
...
func CreateTestSecretWithRevealWindow(id string, threshold int64, state string, revealStart, revealEnd int64) *blockchain.Secret {
...
func CreateTestAssignment(guardianAddr string, status string) blockchain.GuardianAssignment {
	return blockchain.GuardianAssignment{
...
func CreateTestAssignmentWithRealEncryption(secretID, guardianAddr string, status string, guardianPublicKey [32]byte) (blockchain.GuardianAssignment, error) {
	...
	if err != nil {
		return blockchain.GuardianAssignment{}, fmt.Errorf("failed to encrypt test share: %w", err)
	}

	// Generate real HMAC using crypto library
	hmacBytes, err := crypto.GenerateHMAC(secretID, guardianAddr, []byte(shareData))
	if err != nil {
		return blockchain.GuardianAssignment{}, fmt.Errorf("failed to generate HMAC: %w", err)
	}
