...
// rotationHeight).
func setupRotatedGuardian(t *testing.T, chain *mocks.MockChain, rotationHeight int64) (cfg *config.Config, oldPair, newPair *crypto.KeyPair) {
	t.Helper()
	...
	chain.SetKeyHistory(cfg.GuardianAddress, []blockchain.KeyEpoch{
...
func TestEpochKeyResolver_DerivationAndLoading(t *testing.T) {
	...
	cfg, oldPair, newPair := setupRotatedGuardian(t, chain, 100)
	client := mocks.NewMockClient(chain, logger)
...
func TestEpochKeyResolver_MissingRetiredKeyReported(t *testing.T) {
	logger := mocks.CreateTestLogger()
	chain := mocks.NewMockChain()
	cfg, _, _ := setupRotatedGuardian(t, chain, 100)
	client := mocks.NewMockClient(chain, logger)
	resolver := NewEpochKeyResolver(cfg, client, logger)
	ctx := context.Background()
...
func TestDecryptShare_ResolvesRetiredEpoch(t *testing.T) {
	...
	preRotation := &blockchain.Secret{ID: "pre-rotation-secret", CreatedAt: 50}
	...
	postRotation := &blockchain.Secret{ID: "post-rotation-secret", CreatedAt: 150}
...
func TestDecryptShare_TrialFallbackCoversStaleDerivation(t *testing.T) {
	logger := mocks.CreateTestLogger()
	chain := mocks.NewMockChain()
	cfg, _, newPair := setupRotatedGuardian(t, chain, 100)
	client := mocks.NewMockClient(chain, logger)
	service := NewShareRevealService(cfg, client, logger)
	...
	stale := &blockchain.Secret{ID: "stale-height-secret", CreatedAt: 0}
