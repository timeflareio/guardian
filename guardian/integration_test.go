...
import (
	...
	"github.com/timeflareio/guardian/guardian/mocks"
)
...
func TestEndToEndSecretFlow(t *testing.T) {
	...
	chain := mocks.NewMockChain()
	mockClient := mocks.NewMockClient(chain, logger)
	...
	err = chain.AddAssignmentWithRealEncryption(secret.ID, mocks.TestGuardianAddress, "ASSIGNMENT_STATUS_PROPOSED", publicKey)
	require.NoError(t, err)
	...
	chain.UpdateAssignmentStatus(secret.ID, mocks.TestGuardianAddress, "ASSIGNMENT_STATUS_ACCEPTED")
	chain.SetSecretState(secret.ID, "pending")

	// Advance to reveal window
	chain.AdvanceToHeight(100) // exactly at reveal start
...
func TestMultipleSecretsParallel(t *testing.T) {
	...
	chain.AdvanceToHeight(currentHeight)
	...
	for i, s := range secrets {
		...
		chain.AddSecret(secret)
		err = chain.AddAssignmentWithRealEncryption(s.id, mocks.TestGuardianAddress, s.asgn, publicKey)
		require.NoError(t, err)
	}
...
func TestBlockProgression(t *testing.T) {
	...
	err = chain.AddAssignmentWithRealEncryption(secretID, mocks.TestGuardianAddress, "ASSIGNMENT_STATUS_ACCEPTED", publicKey)
	require.NoError(t, err)
	...
	mockClient.ClearCalls()
	chain.AdvanceToHeight(250) // Past reveal end
	err = service.processSecrets(ctx)
	require.NoError(t, err)
...
func TestRevealWindowExpiry(t *testing.T) {
	...
	chain.AddSecret(secret)
	chain.AddAssignment(secretID, mocks.TestGuardianAddress, "ASSIGNMENT_STATUS_ACCEPTED")
	...
	chain.AdvanceBlocks(100) // Trigger cleanup
	err = service.processSecrets(ctx)
	require.NoError(t, err)
...
func TestCacheEvictionDuringProcessing(t *testing.T) {
	...
	chain.AddSecret(secret)
	chain.AddAssignment(secretID, mocks.TestGuardianAddress, "ASSIGNMENT_STATUS_PROPOSED")

	// Initialize cache
	err := cache.Initialize(ctx, mockClient, mocks.TestGuardianAddress, chain.GetCurrentHeight())
	require.NoError(t, err)
...
func TestServiceRestartRecovery(t *testing.T) {
	...
	err = service1.activeSecretCache.Initialize(ctx, mockClient, mocks.TestGuardianAddress, chain.GetCurrentHeight())
	require.NoError(t, err)
	...
	err = service2.activeSecretCache.Initialize(ctx, mockClient, mocks.TestGuardianAddress, chain.GetCurrentHeight())
	require.NoError(t, err)
...
func TestErrorRecovery(t *testing.T) {
	...
	err := service.activeSecretCache.Initialize(ctx, mockClient, mocks.TestGuardianAddress, chain.GetCurrentHeight())
	require.NoError(t, err)
...
func TestPerformanceUnderLoad(t *testing.T) {
	...
	for i := 0; i < secretCount; i++ {
		...
		chain.AddSecret(secret)
		err = chain.AddAssignmentWithRealEncryption(secretID, mocks.TestGuardianAddress, assignmentStatus, publicKey)
		require.NoError(t, err)
	}
	...
	revealSecrets := service.activeSecretCache.GetSecretsNeedingReveal(chain.GetCurrentHeight())
...
func TestConcurrentSecretProcessing(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		secret := mocks.CreateTestSecret(secretID, 1, "awaiting_acceptance")
		chain.AddSecret(secret)
		err = chain.AddAssignmentWithRealEncryption(secretID, mocks.TestGuardianAddress, "ASSIGNMENT_STATUS_PROPOSED", publicKey)
		require.NoError(t, err)
	}
