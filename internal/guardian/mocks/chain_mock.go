...
// Ensure MockClient implements chain.ClientInterface
var _ chain.ClientInterface = (*MockClient)(nil)

// MockChain simulates blockchain state and block progression
type MockChain struct {
	currentHeight  int64
	secrets        map[string]*chain.Secret
	guardians      map[string]*chain.Guardian
	keyHistories   map[string][]chain.KeyEpoch
...
func NewMockChain() *MockChain {
	return &MockChain{
		currentHeight:  1,
		secrets:        make(map[string]*chain.Secret),
		guardians:      make(map[string]*chain.Guardian),
		keyHistories:   make(map[string][]chain.KeyEpoch),
		blockCallbacks: make([]func(height int64), 0),
	}
}

// SetKeyHistory sets a guardian's key-epoch history (epoch order).
func (m *MockChain) SetKeyHistory(address string, history []chain.KeyEpoch) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockChain) GetKeyHistory(address string) []chain.KeyEpoch {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *MockChain) AddSecret(secret *chain.Secret) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockChain) GetSecret(secretID string) (*chain.Secret, bool) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *MockChain) AddGuardian(guardian *chain.Guardian) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockChain) GetGuardian(address string) (*chain.Guardian, bool) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *MockChain) GetSecretsForGuardian(guardianAddr string) []*chain.Secret {
	m.mu.RLock()
	defer m.mu.RUnlock()

	var result []*chain.Secret
...
// MockClient implements chain.Client for testing
...
// Ping implements chain.ClientInterface
...
// Close implements chain.ClientInterface
func (m *MockClient) Close() error {
	return nil
}

// SignerAddress implements chain.ClientInterface
func (m *MockClient) SignerAddress() string {
	return "tmflr1mocksigner"
}

// GetGuardian implements chain.Client
func (m *MockClient) GetGuardian(ctx context.Context, address string) (*chain.Guardian, error) {
...
// GetSecret implements chain.Client
func (m *MockClient) GetSecret(ctx context.Context, secretID string) (*chain.Secret, error) {
...
// ListSecretsForGuardian implements chain.Client
func (m *MockClient) ListSecretsForGuardian(ctx context.Context, guardianAddress string) ([]chain.Secret, error) {
	...
	result := make([]chain.Secret, len(secrets))
...
// GetCurrentBlockHeight implements chain.Client
...
// GetGuardianAddress implements chain.Client
...
// GetBalance implements chain.Client
func (m *MockClient) GetBalance(ctx context.Context, address, denom string) (*chain.Balance, error) {
...
// GuardianConfirmShares implements chain.Client
...
// GuardianRevealShare implements chain.Client
...
// GuardianRegister implements chain.ClientInterface
func (m *MockClient) GuardianRegister(ctx context.Context, opts chain.GuardianRegisterOptions) (string, error) {
	// For testing, just return a mock transaction hash
	return fmt.Sprintf("tx_register_%d", time.Now().UnixNano()), nil
}

// GuardianUpdate implements chain.ClientInterface
func (m *MockClient) GuardianUpdate(ctx context.Context, opts chain.GuardianUpdateOptions) (string, error) {
	return fmt.Sprintf("tx_update_%d", time.Now().UnixNano()), nil
}

// GetGuardianKeyHistory implements chain.ClientInterface. An empty
// history models pre-rotation state — resolvers treat it as single-epoch.
func (m *MockClient) GetGuardianKeyHistory(ctx context.Context, address string) ([]chain.KeyEpoch, error) {
	if m.getGuardianError != nil {
		return nil, m.getGuardianError
	}
...
// GuardianRotateKey implements chain.ClientInterface: appends the next
...
func (m *MockClient) GuardianRotateKey(ctx context.Context, newKey []byte) (string, error) {
	...
	m.chain.keyHistories[address] = append(m.chain.keyHistories[address], chain.KeyEpoch{
		Epoch:               newEpoch,
		PublicKey:           newKey,
