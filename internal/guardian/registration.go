...
func NewRegistrationManager(cfg *config.Config, client chain.ClientInterface, logger *zap.Logger) *RegistrationManager {
...
func (rm *RegistrationManager) RegisterWithOptions(ctx context.Context, opts *RegistrationOptions) error {
	...
	if opts.IsUpdate {
		accepting := true
		txHash, err = rm.client.GuardianUpdate(ctx, chain.GuardianUpdateOptions{
		...
		txHash, err = rm.client.GuardianRegister(ctx, chain.GuardianRegisterOptions{
