...
type EpochKeyResolver struct {
	...
	mu      sync.RWMutex
	history []chain.KeyEpoch // epoch order; nil until first fetch
}

// NewEpochKeyResolver creates a resolver for the configured guardian.
func NewEpochKeyResolver(cfg *config.Config, client chain.ClientInterface, logger *zap.Logger) *EpochKeyResolver {
...
func maxEpoch(history []chain.KeyEpoch) int64 {
	max := int64(-1)
...
func (r *EpochKeyResolver) snapshot(ctx context.Context) ([]chain.KeyEpoch, error) {
	r.mu.RLock()
	history := r.history
	r.mu.RUnlock()
	if history != nil {
